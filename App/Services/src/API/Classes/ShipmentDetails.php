...
class ShipmentDetails implements Normalize
{
    ...
    private $cashAdditionalDescription;
     ...
     * @return Money
     */
    public function getCashAdditionalDescription()
    {
        return $this->cashAdditionalDescription;
    }

    /**
     * Additional Cash that can be required for miscellaneous purposes.
     * @param Money $cashAdditionalDescription
     * @return ShipmentDetails
     */
    public function setCashAdditionalDescription(Money $cashAdditionalDescription): ShipmentDetails
    {
    ...
    /**
     * Additional Cash that can be required for miscellaneous purposes.
     * @param Money $collectAmount
     * @return ShipmentDetails
     */
    public function setCollectAmount(Money $collectAmount): ShipmentDetails
    {
     ...
     * @return ShipmentDetails
     */
    public function setPaymentOptions(string $paymentOptions): ShipmentDetails
    {
    ...
    public function createFromArray($details)
    {
        if (isset($details['dimensions']['length'])) {
            $this->setDimensions((new Dimension())->createFromArray($details['dimensions']));
        }
        $this->setActualWeight((new Weight())->createFromArray($details['actualWeight']));
        $this->setDescriptionOfGoods($details['descOfGoods']);
        $this->setGoodsOriginCountry($details['origin']);
        $this->setNumberOfPieces($details['numberOfPieces']);
        $this->setProductGroup($details['productGroup']);
        $this->setProductType($details['productType']);
        $this->setPaymentType($details['paymentType']);
        if (isset($details['customsValue']['amount'])) {
            $this->setCustomsValueAmount((new Money())->createFromArray($details['customsValue']));
        }
        if (isset($details['cashOnDelivery']['amount'])) {
            $this->setCashOnDeliveryAmount((new Money())->createFromArray($details['cashOnDelivery']));
        }
        if (isset($details['insurance']['amount'])) {
            $this->setInsuranceAmount((new Money())->createFromArray($details['insurance']));
        }
        if (isset($details['additional']['amount'])) {
            $this->setCashAdditionalAmount((new Money())->createFromArray($details['additional']));
        }
        if (isset($details['collect']['amount'])) {
            $this->setCollectAmount((new Money())->createFromArray($details['collect']));
        }
        if (isset($details['services'])) {
            $this->setServices(implode(',', $details['services']));
        }
        if (isset($details['items']['quantity'])) {
            $this->setItems(array_map(function ($item) {
                return (new ShipmentItem())->createFromArray($item);
            }, $details['items']));
        }
        return $this;
    }
