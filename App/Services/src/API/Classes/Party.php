...
//    }
    ...
    public function createFromArray($party)
    {
        if ($party['reference1']) {
            $this->setReference1($party['reference1']);
        }

        if ($party['reference2']) {
            $this->setReference2($party['reference2']);
        }

        $this->setAccountNumber($party['accountNumber']);
        $this->setPartyAddress((new Address())->createFromArray($party['address']));
        $this->setContact((new Contact())->createFromArray($party['contact']));
