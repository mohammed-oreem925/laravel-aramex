...
class Shipment implements Normalize
{
    ...
    public function createFromArray($shipment): Shipment
    {
        if ($shipment['reference1']) {
        $this->setReference1($shipment['reference1']);
        }
        if ($shipment['reference2']) {
            $this->setReference2($shipment['reference2']);
        }
        if ($shipment['reference3']) {
            $this->setReference3($shipment['reference3']);
        }
        $this->setShipper((new Party())->createFromArray($shipment['shipper']));
        $this->setConsignee((new Party())->createFromArray($shipment['consignee']));
        if ($shipment['thirdParty'] && isset($shipment['thirdParty']['accountNumber'])) {
            $this->setThirdParty((new Party())->createFromArray($shipment['thirdParty']));
        }
        $this->setShippingDateTime(strtotime($shipment['shippingDateTime']));
        $this->setDueDate(strtotime($shipment['dueDate']));
        if ($shipment['comments']) {
            $this->setComments($shipment['comments']);
        }
        if ($shipment['pickupLocation']) {
            $this->setPickupLocation($shipment['pickupLocation']);
        }
        if ($shipment['operationsInstructions']) {
            $this->setOperationsInstructions($shipment['operationsInstructions']);
        }
        if ($shipment['accountingInstructions']) {
            $this->setAccountingInstructions($shipment['accountingInstructions']);
        }
        $this->setDetails((new ShipmentDetails())->createFromArray($shipment['details']));
        if ($shipment['attachments']) {
            foreach ($shipment['attachments'] as $attachment) {
                $this->addAttachment((new Attachment())->createFromArray($attachment));
            }
        }
        if ($shipment['foreignHAWB']) {
            $this->setForeignHAWB($shipment['foreignHAWB']);
        }
        $this->setTransportType($shipment['transportType']);
        if ($shipment['number']) {
            $this->setNumber($shipment['number']);
        }
        if ($shipment['pickupGUID']) {
            $this->setPickupGUID($shipment['pickupGUID']);
        }
