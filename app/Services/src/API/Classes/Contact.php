...
class Contact implements Normalize
{
    ...
    public function createFromArray($contact)
    {
        if ($contact['department']) {
            $this->setDepartment($contact['department']);
        }
        $this->setPersonName($contact['name']);
        if ($contact['title']) {
            $this->setTitle($contact['title']);
        }
        $this->setCompanyName($contact['company']);
        $this->setPhoneNumber1($contact['phone1']);
        if ($contact['phoneExt1']) {
            $this->setPhoneNumber1Ext($contact['phoneExt1']);
        }
        if ($contact['phone2']) {
            $this->setPhoneNumber2($contact['phone2']);
        }
        if ($contact['phoneExt2']) {
            $this->setPhoneNumber2Ext($contact['phoneExt2']);
        }
        if ($contact['fax']) {
            $this->setFaxNumber($contact['fax']);
        }
        if ($contact['cell']) {
            $this->setCellPhone($contact['cell']);
        }
        $this->setEmailAddress($contact['email']);
        if ($contact['type']) {
            $this->setType($contact['type']);
        }
