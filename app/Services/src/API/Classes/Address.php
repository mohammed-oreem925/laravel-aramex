...
class Address implements Normalize
{
    ...
    public function createFromArray ($address) {
        $this->setLine1($address['line1']);
        if ($address['line2']) {
            $this->setLine2($address['line2']);
        }
        if ($address['line3']) {
        	$this->setLine3($address['line3']);
        }
        if ($address['city']) {
            $this->setCity($address['city']);
        }
        if ($address['stateOrProvinceCode']) {
            $this->setStateOrProvinceCode($address['stateOrProvinceCode']);
        }
        if ($address['postalCode']) {
            $this->setPostCode($address['postalCode']);
        }
